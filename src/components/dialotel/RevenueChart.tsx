          ...
          {live.currentCalls.map((call) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-slate-400">
                Client : {call.client || "Non renseignÃ©"}
              </p>
        ...
        <article className="rounded-2xl border border-slate-800 bg-slate-950/40 p-5">
          ...
          {latestCall ? (
            <>
              ...
              <p className="mt-1 text-sm text-slate-400">
                Client :{" "}
                {latestCall.client || "Non renseignÃ©"}
              </p>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  <p className="text-slate-500">DurÃ©e</p>
                  <p className="mt-1 text-white">
                    {latestCall.duration}
                  </p>
                </div>
        ...
        <article className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
          ...
          {latestMissed ? (
            <>
              ...
              <p className="mt-1 text-sm text-slate-400">
                {latestMissed.time} â€¢ {latestMissed.type}
              </p>

              <p className="mt-4 text-sm text-slate-400">
                ManquÃ© par :{" "}
                <span className="font-medium text-white">
                  {latestMissed.missedBy || "Non renseignÃ©"}
                </span>
              </p>
            </>
          ) : (
            <p className="mt-4 text-sm text-slate-500">
              Aucun appel manquÃ© rÃ©cent.
            </p>
          )}
        </article>
      </div>
